...
const unit = (id: string, title: string, image: string) => ({
  ...
  questions: [{ id: `${id}-question`, prompt: '확인 문제', answer: '정답' }],
  progress: [],
});
...
describe('learning library', () => {
  it('collapses sources and opens the original PDF capture directly from a card', async () => {
    const sources = [
      {
        ...
        subject: '통계',
        category: '기초',
        units: [unit('unit-one', '통계적 사고', '/learning/statistical-thinking.jpg')],
      },
      {
        id: 'source-two',
        title: '두 번째 PDF',
        subject: 'Git',
        category: '환경 구성',
        units: [unit('unit-two', 'Git 작업 흐름', '/learning/git-mental-model.jpg')],
      },
    ];
    vi.stubGlobal(
      'fetch',
      vi.fn((input: RequestInfo | URL) =>
        String(input).endsWith('/learning/due') ? response([]) : response(sources),
      ),
    );
    ...
    expect(capture).toHaveAttribute('src', '/learning/statistical-thinking.jpg');
