...
5. archive 안에 아래 필수 항목이 있는지 확인한다.
   ...
   - 이 기존 프로젝트에서는 `dist/.openai/drizzle/0017_marvelous_blockbuster.sql`만 존재
...
소스 commit만 저장하고 archive를 생략하면 플랫폼이 모노레포의 일반 `build`를 다시 선택하여 Worker와 migration을 누락할 수 있다. 따라서 소스와 archive의 SHA 일치와 archive migration 목록 검사는 배포 불변식이다. 새 Sites 프로젝트는 기존 runtime baseline이 없으므로 이 0017-only staging 규칙 대신 0000부터 전체 migration을 적용하는 별도 bootstrap이 필요하다.
