...
4. API에 JWT와 Slack OIDC secret/env를 주입하고 웹에는 `VITE_API_URL`, `VITE_APP_NAME`만 build-time 주입한다.
...
- 필요하면 `SLACK_ALLOWED_TEAM_ID`, `SLACK_ADMIN_USER_IDS`
...
`pnpm sites:build`는 검증된 `apps/web` production 산출물을 Cloudflare Worker-compatible ESM과 함께 패키징한다. Sites runtime의 `API_ORIGIN`은 별도로 배포한 HTTPS Nest API origin을 가리켜야 한다. Worker가 `/api/v1/*`를 same-origin으로 프록시하므로 브라우저에는 API origin과 Slack client secret이 노출되지 않는다.

`API_ORIGIN`이 없으면 로그인 설정 상태 endpoint만 안전하게 응답하고 Slack 버튼을 비활성화한다. 이는 성공을 가장하는 demo 모드가 아니라 운영 백엔드가 아직 연결되지 않았다는 명시적 상태다. 완전한 운영 전환에는 managed PostgreSQL, Nest API, Slack App credentials, `COOKIE_SECURE=true`, 그리고 Sites `API_ORIGIN`이 모두 필요하다.
