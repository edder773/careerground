...
`.env.example`을 `.env`로 복사한 뒤 세 secret과 DB URL을 변경한다. `pnpm install --frozen-lockfile`, `docker compose up -d postgres`, `pnpm db:migrate`, `pnpm db:seed`, `pnpm dev` 순서로 실행한다.
