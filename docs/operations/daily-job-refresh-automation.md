...
1. 각 출처의 최신 공고 목록을 게시 시각 또는 목록 순서대로 읽는다. 직전 성공 실행에서 기록한 출처별 `highWatermark`를 만나더라도 최근 5일 구간은 다시 읽어 지연 등록·제목 변경·재게시 공고를 포착한다.
...
3. 포괄 제목이나 직군 묶음 공고는 상세 페이지의 직무 목록을 펼쳐 IT 모집 단위가 있는지 확인한다. 제목에 IT 키워드가 없다는 이유만으로 버리지 않는다.
4. `discovered.json`의 각 후보는 정확히 하나의 `ACTIVE_CANDIDATE`, `EXCLUDED`, `UNCERTAIN`, `DUPLICATE` 판정을 가져야 한다. 판정되지 않은 후보가 한 건이라도 있으면 완전성 게이트 실패로 처리한다.
5. 동적 렌더링·로그인·차단 등으로 최신 목록을 끝까지 읽지 못한 출처는 `INCOMPLETE`로 기록하고 후보 0건으로 간주하지 않는다. 해당 출처는 회사명 보완 탐색과 다른 공개 출처 교차검색으로 보완하되, 성공으로 과장하지 않는다.
6. 출처별 `highWatermark`, 재탐색 시작·종료 시각, 읽은 목록 페이지 수, 원시 후보 수, 판정 완료 수를 `reconciliation.json`에 기록한다. 새 high-watermark는 모든 완전성 게이트를 통과한 실행에서만 전진시킨다.
7. 특정 출처 또는 키워드에서만 후보가 집중되면 회사·직무·공공기관·인턴·경력무관 축을 교차해 검색하고, 누락 가능성이 남으면 배포 대신 `UNCERTAIN`으로 보고한다.
