...
P0는 9/9, P1은 44/66, P2는 19/56을 해결했다. 여기서 해결 수는 `완료`, `현재 코드에서 이미 해결됨`, `다른 수정에 통합됨`의 합이다. 운영 자격증명 부족으로 배포만 차단된 항목은 P1 2개/P2 1개이며, 기술적 이유로 미해결인 항목은 P1 20개/P2 36개다. cursor pagination·전면 FK 재구성·rate limit·관측성·자동 백업·공통 Dialog 같은 큰 후속 과제는 완료로 과장하지 않았다.

운영 배포 전에는 D1 export/restore 리허설, 플랫폼 인증 헤더 경계, 운영 정합성 집계, rate limit/관측성 결정이 필요하다. 기존 migration을 파괴적으로 고치지 말고 새 순방향 migration만 추가한다.
